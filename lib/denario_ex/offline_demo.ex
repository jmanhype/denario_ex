defmodule DenarioEx.OfflineDemo do
  @moduledoc """
  Deterministic end-to-end demo for the DenarioEx workflow.
  """
  ...
  @spec run(keyword()) :: {:ok, DenarioEx.t()} | {:error, term()}
  def run(opts \\ []) do
    project_dir = Keyword.get(opts, :project_dir, default_project_dir())
    ...
    with {:ok, denario} <- DenarioEx.new(project_dir: project_dir, clear_project_dir: true),
         ...
         {:ok, denario} <-
           DenarioEx.check_idea(
             denario,
             client: Client,
             semantic_scholar_client: LiteratureClient,
             llm: "openai:gpt-4.1-mini",
             max_iterations: 3
           ),
         {:ok, denario} <-
      ...
      if print?, do: IO.puts(summary(denario, project_dir))
      {:ok, denario}
    end
  end

  @spec summary(DenarioEx.t(), String.t()) :: String.t()
  def summary(denario, project_dir) do
    files =
      [
